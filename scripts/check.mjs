...
const required = [
  ...
  'project/project.json','project/truth-state.json','project/line-reset-closure.json','project/canon-candidates.json','project/canon.json','project/evidence-closure.json',
  'project/character-library.json','project/character-production-sheets.json','project/lora-training-sheets.json',
  'docs/TRUTH_AUDIT_2026-07-11.md','docs/LINE_RESET_CLOSURE_AUDIT_2026-07-11.md','docs/PRODUCTION_APP_RECOVERY_PLAN.md',
...
const [project, truth, lr0Closure, candidates, canon, closure, characters, sheets, loras] = await Promise.all([
  'project/project.json','project/truth-state.json','project/line-reset-closure.json','project/canon-candidates.json','project/canon.json','project/evidence-closure.json','project/character-library.json','project/character-production-sheets.json','project/lora-training-sheets.json'
].map(readJson));
...
const [readme,index,phaseUi,audit,context,recoveryPlan,truthAudit,closureAudit] = await Promise.all(['README.md','index.html','lr1-ui.js','audit-ui.js','lib/context.mjs','docs/PRODUCTION_APP_RECOVERY_PLAN.md','docs/TRUTH_AUDIT_2026-07-11.md','docs/LINE_RESET_CLOSURE_AUDIT_2026-07-11.md'].map((path)=>readFile(new URL(path,root),'utf8')));
...
for (const marker of ['Line Reset abgeschlossen','LR1 · Issue #38','Pilot menschlich auswählen','src="./lr1-ui.js"','id="m1Video"']) if(!index.includes(marker)) throw new Error(`Dashboard marker missing: ${marker}`);
for (const marker of ['LR0 TRUTH RESET','AKTIVES GATE','Issue #38','active_human_decision_required']) if(!phaseUi.includes(marker)) throw new Error(`LR1 UI marker missing: ${marker}`);
...
console.log(JSON.stringify({status:'pass',repository:truth.repository,lineStatus:truth.status,activeGate:'LR1',canonStatus:truth.canon.status,currentEvidenceCoveragePercent:null,productionAppStatus:truth.productArchitecture.productionFoundation.status,lr0Closure:lr0Closure.status}));
