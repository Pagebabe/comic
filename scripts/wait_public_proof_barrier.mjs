...
const targets = [
  {
    ...
    validate: (data) => data.status === 'pass' && data.commit === expectedCommit && data.activeGate === 'LR5' && data.activeWorkPackage === 'LR5.1' && data.route?.endsWith('/audit.html')
  },
  {
    id: 'root-entry',
    path: 'proof/root-entry/root-entry-runtime-evidence.json',
    output: 'root-entry-runtime-evidence.json',
    validate: (data) => data.status === 'pass' && data.commit === expectedCommit && data.trackingIssue === 121 && data.parentIssue === 117 && data.primaryAction === './studio/#cockpit' && data.auditRoute === './audit.html' && data.riccoCandidates === 0 && data.productionReady === false && data.beginnerReady === false && data.imageGenerationAllowed === false && data.growthOsIntegrated === false
  },
  {
    id: 'studio',
    path: 'proof/studio/studio-runtime-evidence.json',
    output: 'studio-runtime-evidence.json',
    validate: (data) => data.status === 'pass' && data.commit === expectedCommit && data.activeGate === 'LR5' && data.activeWorkPackage === 'LR5.1'
  },
  {
    id: 'academy',
    path: 'proof/studio/academy-runtime-evidence.json',
    output: 'academy-runtime-evidence.json',
    validate: (data) => data.status === 'pass' && data.commit === expectedCommit && data.stageCount === 12 && data.creativeApprovalGranted === false && data.finalEpisodeApprovalGranted === false
  },
  {
    id: 'readiness',
    path: 'proof/readiness/academy-readiness-runtime-evidence.json',
    output: 'readiness-runtime-evidence.json',
    validate: (data) => data.status === 'pass' && data.commit === expectedCommit && data.readinessScore === '2/10 CLOSED_VERIFIED · 7 PARTIAL · 1 OPEN' && data.productionReady === false && data.beginnerReady === false && data.creativeApprovalGranted === false
  },
  {
    id: 'cockpit',
    path: 'proof/cockpit/production-cockpit-runtime-evidence.json',
    output: 'cockpit-runtime-evidence.json',
    validate: (data) => data.status === 'pass' && data.commit === expectedCommit && data.trackingIssue === 117 && data.activeGate === 'LR5.1' && data.activeWorkPackage === 88 && data.workspaceCount === 6 && data.riccoCandidates === 0 && data.imageGenerationAllowed === false && data.creativeApprovalGranted === false && data.productionReady === false && data.beginnerReady === false && data.growthOsIntegrated === false
  },
  {
    id: 'academy-status',
    path: 'project/production-academy-status.json',
    output: 'production-academy-status.json',
    validate: (data) => data.status === 'proven_guided_training_ready_novice_acceptance_open' && data.readiness?.productionReady === false && data.readiness?.beginnerReady === false && data.readiness?.observedNoviceRunPassed === false
  },
  {
    id: 'readiness-contract',
    path: 'project/production-readiness-v1.json',
    output: 'production-readiness-v1.json',
    validate: (data) => data.status === 'NOT_PRODUCTION_READY' && data.currentScore?.display === '2/10 CLOSED_VERIFIED · 7 PARTIAL · 1 OPEN' && data.academyBoundary?.productionReady === false && data.academyBoundary?.beginnerReady === false
  },
  {
    id: 'cockpit-contract',
    path: 'project/production-cockpit-v1.json',
    output: 'production-cockpit-v1.json',
    validate: (data) => data.status === 'WORKING_COCKPIT_V1' && data.route === '/studio/#cockpit' && data.trackingIssue === 117 && data.activeGate?.id === 'LR5.1' && data.activeGate?.trackingIssue === 88 && data.nextAllowedStep?.decision === 'CONTRACT_APPROVED_FOR_ONE_CANDIDATE' && data.counts?.riccoCandidates === 0 && data.sections?.length === 6 && Object.values(data.boundaries || {}).every((entry) => entry === false)
  },
  {
    id: 'root-entry-contract',
    path: 'project/root-entry-v1.json',
    output: 'root-entry-v1.json',
    validate: (data) => data.status === 'ROOT_ENTRY_READY_FOR_PROOF' && data.route === '/' && data.trackingIssue === 121 && data.parentIssue === 117 && data.primaryAction?.href === './studio/#cockpit' && data.legacyAudit?.preserved === true && data.legacyAudit?.route === '/audit.html' && data.currentState?.riccoCandidates === 0 && data.currentState?.productionReady === false && data.currentState?.beginnerReady === false && Object.values(data.boundaries || {}).every((entry) => entry === false)
  }
];
