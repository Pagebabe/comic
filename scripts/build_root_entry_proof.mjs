...
const outputPath = outputIndex >= 0 ? resolve(args[outputIndex + 1]) : new URL('../_site/proof/root-entry/', import.meta.url).pathname;
...
for (const target of [{ name: 'desktop', width: 1440, height: 1000 }, { name: 'mobile', width: 390, height: 844 }]) {
  ...
  if (checks.title !== 'Comic Factory · Produktionsstart' || checks.primaryLabel !== 'Produktions-Cockpit öffnen' || checks.primaryHref !== './studio/#cockpit' || checks.entryCards !== 4 || checks.stateCards !== 5 || checks.boundaries !== 4 || !checks.academyLink || !checks.riccoLink || !checks.auditLink || checks.staleLr1Copy || !checks.zeroState || !checks.productionFalse || !checks.beginnerFalse || !checks.imageBlocked || !checks.growthSeparated || checks.scripts !== 0 || checks.buttons !== 0 || checks.images !== 0 || checks.canvas !== 0 || checks.iframe !== 0 || checks.overflow > 2 || external.length) throw new Error(`${target.name} root entry failed: ${JSON.stringify({ checks, external })}`);
  const screenshotName = `root-entry-${target.name}.png`;
  const file = resolve(outputPath, screenshotName);
  await page.screenshot({ path: file, fullPage: true });
  const bytes = await readFile(file);
  targets.push({ ...target, checks, screenshot: screenshotName, sha256: createHash('sha256').update(bytes).digest('hex'), externalRequests: external });
  await page.close();
}
await browser.close();
const manifest = {schemaVersion:1,status:'pass',repository:'Pagebabe/comic',commit,route:new URL(baseUrl).toString(),trackingIssue:121,parentIssue:117,primaryAction:'./studio/#cockpit',auditRoute:'./audit.html',entryCards:4,stateCards:5,boundaries:4,externalRequests:0,scriptCount:0,buttonCount:0,imageCount:0,canvasCount:0,iframeCount:0,riccoCandidates:0,productionReady:false,beginnerReady:false,imageGenerationAllowed:false,growthOsIntegrated:false,targets,generatedAt:new Date().toISOString()};
await writeFile(resolve(outputPath, 'root-entry-runtime-evidence.json'), `${JSON.stringify(manifest, null, 2)}\n`);
