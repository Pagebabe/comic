...
const statusPath = join(root, 'outputs', 'comfyui', 'pilot', 'pilot-render-status.json');
const outRoot = join(root, 'outputs', 'comfyui', 'pilot');
const historyReportPath = join(outRoot, 'pilot-history-report.json');
...
function readJson(path, label) {
  if (!existsSync(path)) {
    ...
    console.error('Run first: node scripts/createPilotRenderStatus.mjs');
    process.exit(1);
  }
...
function assertSafeToPoll() {
  if (!shouldPoll) return { ok: false, reason: 'Dry run only. Add --poll to query ComfyUI history.' };
  if (!allowHistory) return { ok: false, reason: 'Missing COMFYUI_ALLOW_HISTORY=1.' };
  return { ok: true, reason: 'History polling enabled by explicit flag and environment.' };
}
...
const status = readJson(statusPath, 'render status');
...
const report = {
  id: 'pilot_comfy_history_report',
  createdAt: new Date().toISOString(),
  endpoint,
  sourceStatus: 'outputs/comfyui/pilot/pilot-render-status.json',
  mode: pollGate.ok ? 'history_poll_enabled' : 'dry_run_blocked',
  pollGate,
  ...
  previews,
  results,
  nextStep: pollGate.ok ? 'Convert completed history outputs into review manifests.' : 'Set COMFYUI_ALLOW_HISTORY=1 and pass --poll after real prompts are queued.'
};

mkdirSync(outRoot, { recursive: true });
writeFileSync(historyReportPath, JSON.stringify(report, null, 2), 'utf8');

console.log('wrote outputs/comfyui/pilot/pilot-history-report.json');
