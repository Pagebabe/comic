...
test('production readiness remains honest and measurable',async()=>{
  const [readiness,status]=await Promise.all([json('project/production-readiness-v1.json'),json('project/production-academy-status.json')]);
  assert.equal(readiness.schemaVersion,2);
  assert.equal(readiness.status,'NOT_PRODUCTION_READY');
  assert.deepEqual(readiness.currentScore,{closedVerified:2,partial:7,open:1,total:10,display:'2/10 CLOSED_VERIFIED · 7 PARTIAL · 1 OPEN'});
  ...
  for(const value of ['productionReady','beginnerReady','creativeApprovalGranted','imageGenerationAllowed','growthOsIntegrated']) assert.equal(readiness.academyBoundary[value],false);
...
test('novice acceptance requires a real observed run',async()=>{
...
test('Academy exposes readiness without a second onboarding route',async()=>{
  const [app,component,css,protocol,evidence]=await Promise.all([
    read('studio-app/src/App.tsx'),read('studio-app/src/AcademyReadiness.tsx'),read('studio-app/src/academy-readiness.css'),read('docs/NOVICE_ACCEPTANCE_PROTOCOL.md'),read('docs/PRODUCTION_ACADEMY_EVIDENCE.md')
  ]);
  ...
  assert.doesNotMatch(evidence,/technisch einsatzbereit und anfängerbedienbar/);
});

test('required readiness files are present',async()=>{
  for(const file of ['project/production-readiness-v1.json','project/novice-acceptance-template.json','docs/NOVICE_ACCEPTANCE_PROTOCOL.md','studio-app/src/AcademyReadiness.tsx','studio-app/src/academy-readiness.css']) await access(new URL(file,root));
});
