...
test('absolute timing contains ten non-overlapping locked dialogue cues', () => {
  ...
  assert.equal(cues[1].startSeconds, 7.35);
  assert.equal(cues[1].endSeconds, 10.3);
  assert.equal(cues[7].startSeconds, 32.55);
  assert.equal(cues[7].endSeconds, 35.45);
...
test('timing report preserves 45.5 seconds and passes the reading-speed gate', () => {
  const cues = buildTimingCues(blueprint);
  const report = buildTimingReport(blueprint, cues);
  assert.equal(report.status, 'temporary_timing_only');
  assert.equal(report.pacingStatus, 'pass');
  assert.equal(report.targetDurationSeconds, 45.5);
  assert.equal(report.panelCount, 8);
  assert.equal(report.subtitleCueCount, 10);
  assert.equal(report.maximumAllowedCharactersPerSecond, 17);
  assert.ok(report.maximumCharactersPerSecond <= 17);
  assert.deepEqual(report.cuesAboveMaximumCharactersPerSecond, []);
  assert.deepEqual(report.cuesAbove17CharactersPerSecond, []);
  assert.equal(report.automaticVoiceApproval, false);
  assert.equal(report.automaticCanonApproval, false);
  assert.ok(report.spokenSeconds > 20);
  assert.ok(report.silentOrReactionSeconds > 20);
...
test('CLI export writes only timing SRT and a non-canonical passing report', async () => {
  ...
  assert.equal(report.pacingStatus, 'pass');
  assert.equal(report.maximumAllowedCharactersPerSecond, 17);
  assert.equal(report.cuesAboveMaximumCharactersPerSecond.length, 0);
  assert.equal(report.automaticVoiceApproval, false);
  assert.equal(report.automaticCanonApproval, false);
