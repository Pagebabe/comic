...
test('EpisodePackage validation is strict and preserves shadow provenance', () => {
  ...
  assert.throws(() => validateEpisodePackage({ ...syntheticEpisodePackage, title: '' }), GrowthValidationError);
  assert.throws(
    () => validateEpisodePackage({
      ...syntheticEpisodePackage,
...
test('publish state machine rejects skipped gates', () => {
  ...
  assert.throws(() => transitionPublishJob(job, 'SIMULATED', FIXED_TIMESTAMP, 'SKIP'), /Invalid publish job transition/);
