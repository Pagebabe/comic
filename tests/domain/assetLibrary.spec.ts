...
test('builds asset library items with panel and job metadata', () => {
...
test('summarizes finals variants local/url images and job links', () => {
  const items = buildAssetLibraryItems([
    makeImage({ id: 'final', selected: true, generationJobId: 'job_1', imageUrl: 'data:image/png;base64,abc', source: 'local_file' }),
    makeImage({ id: 'variant', selected: false, imageUrl: '/generated/panel_001.png', source: 'public_asset' })
  ], [makeJob({ id: 'job_1' })]);
...
test('filters by panel final state source job link and query', () => {
  const items = buildAssetLibraryItems([
    makeImage({ id: 'final', panelId: 'panel_001', selected: true, generationJobId: 'job_1', source: 'public_asset', notes: 'keeper' }),
    makeImage({ id: 'variant', panelId: 'panel_002', selected: false, source: 'manual_url', notes: 'trash' })
...
test('builds readable asset library report', () => {
