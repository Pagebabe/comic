...
export default function App() {
  ...
  return <div className="app" data-testid="studio-foundation">
    <header className="topbar">
      <div><p className="eyebrow">COMIC FACTORY · RECOVERY LINE</p><strong>Production Studio</strong></div>
    ...
    {view === 'loop' ? <ProductionLoop />
      ...
      : <main className="shell" id="foundation">
        ...
        <section className="status-grid" id="status">
          <article className="panel"><p className="eyebrow">LR5.1 · AKTIVER RICCO-VERTRAG</p><h2>Erst Vertrag prüfen, dann genau ein Kandidat</h2><ul className="check-list"><li><b>✓</b><span>7/7 Quellen und fünf Konflikte dokumentiert</span></li><li><b>✓</b><span>zehn Reviewtests und Kandidatenlimit 1</span></li><li><b>✓</b><span>Bildgenerierung, Batch und LoRA blockiert</span></li><li><b>✓</b><span>keine automatische Masterzuweisung</span></li></ul><p className="boundary">Die Academy ändert diesen Vertrag nicht. Der nächste kreative Schritt bleibt eine ausdrückliche menschliche Entscheidung.</p><a className="loop-link" href="#lr5-ricco">Ricco-Vertrag öffnen</a></article>
          <article className="panel"><p className="eyebrow">PRODUCTION ACADEMY · ISSUES #94 UND #95</p><h2>Morgen ohne Vorwissen strukturiert anfangen</h2><ul className="check-list"><li><b>✓</b><span>zwölf gesperrte Stufen</span></li><li><b>✓</b><span>Übungs- und Echtmodus mit Human Review</span></li><li><b>✓</b><span>Resume, Notizen und Fortschrittsexport</span></li><li><b>✓</b><span>Handbuch, Video-Drehbuch und 13 Vorlagen</span></li><li><b>!</b><span>beobachteter Nullwissen-Lauf und reale Episode offen</span></li></ul><p className="boundary">Academy technisch bewiesen. Anfänger-Abnahme und Produktionsreife bleiben offen, bis die fehlenden Readiness-Gates geschlossen sind.</p><a className="loop-link" href="#academy">Serie starten und Readiness prüfen</a></article>
          <article className="panel warning" data-testid="not-restored"><p className="eyebrow">LR5 · OFFENE PRODUKTIONSWAHRHEIT</p><h2>Visual-, Set- und Voice-Locks</h2><ul><li>Ricco-Vertrag menschlich prüfen</li><li>exakt einen versionierten Kandidaten erzeugen</li><li>Ansichten und Ausdrücke sichtbar reviewen</li><li>danach weitere Figuren, Orte und Stimmen bearbeiten</li><li>jede Freigabe an Version und Hash binden</li></ul><p className="boundary">Character-Master 0/4, Location-Master 0/4, Stimmen 0/3, fertige Episoden 0. Keine automatische Freigabe und kein Massenrendern.</p></article>
        </section>
      </main>}

    <footer><span>Repository: {truth.repository}</span><span>Route: {foundation.route}{view === 'loop' ? '#loop' : view === 'pilot' ? '#pilot-fire-test' : view === 'ricco' ? '#lr5-ricco' : view === 'academy' ? '#academy' : ''}</span><span>Academy technisch bewiesen · Anfänger-Abnahme offen · kreative Master bleiben REVIEW_REQUIRED</span></footer>
  </div>;
}
