...
export function localCommandReply(message) {
  ...
  switch (command) {
    case '/status':
      return { reply: 'Aktive Linie: M1R Canon & Asset Recovery ist geprüft, Visual Preproduction ist bereit und der EP001-Animatic-Blueprint ist mit 8 Panels und 45,5 Sekunden gesperrt. Story und 4/4 Text-Character-Bibles bleiben unverändert. Der lokale Scan fand 0 vertrauenswürdige visuelle Master. Der M1-Clip bleibt ein Technikbeweis. Bildgenerierung ist pausiert; das Blueprint ist noch kein gerendertes Animatic.' };
    case '/next':
      return { reply: 'Nächster kontrollierter Schritt ohne Bilder: neutraler Dialog-Readthrough gegen die 45,5-Sekunden-Struktur, Untertitellängen und Reaktionspausen prüfen. Dabei keine Stimme als Canon markieren. Wenn der Bildgenerator verfügbar ist, ausschließlich Ricco-Silhouetten und ein Ricco-Character-Sheet nach project/visual-preproduction.json erzeugen. Keine Ersatzpipeline und kein paralleler Cast-Batch.' };
    case '/characters':
    ...
    case '/plan':
      return { reply: 'M0 Bestand/Neustart ✓ → M1 technischer Pipelinebeweis ✓ → M1R Text-Canon 4/4 ✓, Recovery geprüft ✓, Visual-Briefs 8/8 ✓, EP001 Blueprint 8 Panels / 45,5 s ✓, Bildgenerierung pausiert → neutraler Timing-Readthrough → Ricco zuerst → visuelle Masterreferenzen → M2 kanonische Mini-Szene → M3 Character/Location Lock → M4 gerendertes Acht-Beat-Animatic → M5 Pilotfolge → M6 Wiederholung → M7 gezielte Automation.' };
    case '/help':
