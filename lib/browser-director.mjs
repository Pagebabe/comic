...
export function localCommandReply(message) {
  ...
  switch (command) {
    case '/status': return { reply: 'LR0 bis LR3 sind geschlossen. Das Zimmer ist ausgewählt. Studio Foundation unter /studio/ und neutraler Produktionsloop sind öffentlich bewiesen; LR3 bestand 9/9 Stationen inklusive tatsächlicher Zustandslöschung und hashgleichem Restore. Aktives Gate: LR4 Selected-Pilot-Fire-Test, Issue #76. Das ausgewählte Pilotpaket ist noch nicht bewiesen. Evidence bleibt partiell ohne Prozentzahl. Character-Master 0/4, Sets 0/4, Stimmen 0/3, fertige Episode nein.' };
    case '/next': return { reply: 'Nächster kontrollierter Schritt: autorisierte Das-Zimmer-Quellen einzeln inventarisieren und genau einen SelectedPilotEpisodePackage-Vertrag festlegen. Dialoge, Panels, Timingdaten, Bibles, Visuals und Stimmen bleiben REVIEW_REQUIRED. Danach Import, technische Review, QA, Lettering, Package, vollständige Zustandslöschung und hashgleichen Restore beweisen. Kein Blind-Merge, keine Bildgenerierung und keine externe automatische Ausführung.' };
    case '/characters': return { reply: 'Ricco, Basti Prenzl, Jule und Don Miau gehören zur ausgewählten Pilotlinie Das Zimmer. Ihre Bibles bleiben Detail-Ausgangsmaterial mit offenem Review. Visuelle Master bleiben 0/4; LR4 transportiert das Pilotpaket, erteilt aber keine Detail-, Visual- oder Voice-Freigabe.' };
    case '/plan': return { reply: 'LR0 Truth Reset ✓ → LR1 Das Zimmer ausgewählt ✓ → LR2 Studio Foundation öffentlich bewiesen ✓ → LR3 neutraler Studio-bis-Restore-Loop öffentlich bewiesen ✓ → LR4 Das-Zimmer-Fire-Test aktiv → LR5 Visual-, Set- und Voice-Locks → LR6 erster echter Pilot.' };
    ...
    case '/render': {
      ...
      return { reply: `${isRender ? 'Renderauftrag' : 'Arbeitspaket'} als GitHub-Entwurf vorbereitet: ${title}${isRender ? '\nAusführung ist durch LR4 und fehlende Visual-Locks blockiert.' : ''}`, issueDraftUrl: buildIssueDraft({ type: isRender ? 'RENDER REQUEST' : 'WORK PACKET', title, details }) };
    }
    default: return null;
  }
}
